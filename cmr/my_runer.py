...
class Runner(object):
    ...
    def demo(self):
        ...
        self.model.eval()
        # breakpoint()
        dummy_input = torch.zeros(2, 3,args.size,args.size, requires_grad=True).to(self.device)
        print('cvt to onnx')
        torch.onnx.export(self.model,         # model being run 
            dummy_input,       # model input (or a tuple for multiple inputs) 
            "/home/xxx/datasets/realtime_test_out/onnx/handmesh_2hand.onnx",       # where to save the model  
        ...
        # import onnx
        # onnx_model = onnx.load('/home/xxx/datasets/realtime_test_out/onnx/handmesh_2hand.onnx') # 加载 onnx 模型
        ...
        # path = '/home/xxx/datasets/realtime_test_out/onnx/handmesh_2hand.onnx'
        ...
        # #forward model
        # res = sess.run([output_name], {input_name: dummy_input.float().detach().cpu().numpy()})
        # breakpoint()
        # out = np.array(res)
        ...
        #         # K[0, 2] = args.size // 2
        #         # K[1, 2] = args.size // 2
        #         # input = torch.zeros(1, 3,args.size,args.size, requires_grad=True).to(self.device)
        #         # breakpoint()
        #         torch.cuda.synchronize()
        #         start = time.time()
        ...
        #         save_mesh(os.path.join(args.out_dir, 'demo4', image_name + '_mesh.ply'), vertex, self.faces[0])
        #         # breakpoint()
        ...
        #         # onnx 网络输出
        #         # onnx_out = sess.run(None, {'modelInput': to_numpy(input)})
        ...
        #         # save_mesh(os.path.join(args.out_dir, 'demo2', image_name + '_mesh.ply'), vertex, self.faces[0])
